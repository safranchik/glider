...
class VoteMatrix:
    ...
    def __init__(self, matrix=None, length=0):

        self._matrix = matrix

        if self._matrix is None:
            assert length > 0
            self._matrix = pd.DataFrame(index=np.arange(length), columns=[], dtype=int)
        elif not isinstance(matrix, pd.DataFrame):
            self._matrix = pd.DataFrame(matrix)

        self.votes = self._matrix.values
    ...
    def add_lf(self, lf_name, lf_votes):
        ...
        if lf_name in self._matrix:
            raise ValueError('LF name "%s" is already taken.' % lf_name)

        self._matrix[lf_name] = lf_votes
        self.__update_votes()
    ...
    def evaluate_predictions(self, labels, weighting, class_to_id):

        cols = ['Correct', 'Incorrect', 'Accuracy']

        if not len(self._matrix.columns):
            return pd.DataFrame(columns=cols)

        np.random.seed(len(self.votes))     # reuse the same seed to replicate results

        label_distribution = weighting(self.votes, class_to_id)
